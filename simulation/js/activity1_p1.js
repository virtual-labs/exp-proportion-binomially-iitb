function activity1_p1() {
    ...
    btn && btn.remove();
    let btn_txt = get_collapse_btn_text('Selecting alpha value', 'act1-div-alpha');
    maindiv.innerHTML += `
      ${btn_txt}
      <div class="collapse divide" id="act1-div-alpha">
      ...
      <div>
         ...
         <div class='row' id='act1-dd-div'>
            <div class='col-6'>Enter Alpha Value</div>
            <div class='col-6'>
               <select class='form-select fs-16px'  id='act1-alpha-inp'></select>
            </div>
            <br><br>
            <div>
               <button class='btn btn-info std-btn' onclick='a1_set_alpha();' id='act1-p1-btn-1' >Next</button>
            </div>
   ...
   `;
    ...
    hide_all_steps();
    setTimeout(() => {
        show_step('act1-div-alpha');
    }, 150);
    a1_load_alpha_dd();
}
function a1_load_alpha_dd() {
...
function a1_set_alpha() {
    let inp = (document.getElementById('act1-alpha-inp'));
    let next_btn = (document.getElementById('act1-p1-btn-2'));
    if (inp.value != '0') {
        alpha_a1 = parseFloat(inp.value);
        // z_alpha_a1 = 0;
        // let x = z1;
        ...
        // if (x1 == x2) {
        // 	z_alpha_a1 = z_table[indx][alpha_a1];
        // } else {
        // 	y1 = z_table[indx][alpha_a1];
        // 	y2 = z_table[indx - 1][alpha_a1];
        // 	let m = (y1 - y2) / (x1 - x2);
        // 	let c = y1 - m * x1;
        // 	z_alpha_a1 = parseFloat((m * x + c).toFixed(3));
        // }
        // console.log(x1, x2);
        // console.log(y1, y2);
        // console.log('z_alpha', z_alpha_a1);
        let s_2 = (document.getElementById('act1-dd-div'));
        console.log('alpha', inp.value);
        s_2.innerHTML = `<h5>Selected &alpha; = ${alpha_a1.toFixed(2)}</h5>`;
